...
function set_param!(scratch, θ)
    ...
    scratch.par.K = inv_logit(θ.log_K)
...
out = adaptive_pmmh(theta0, prior, state, nsamples; thin = 100,
...
chn_pmcmc = let
    ...
    MCMCChains.Chains(θ', collect(fieldnames(ModelParam)))
end
```
...
```{julia}
function retrodict_pmcmc(dat)
    r, K, σ_o, x₀ = vec(Array(sample(dat, 1)))
