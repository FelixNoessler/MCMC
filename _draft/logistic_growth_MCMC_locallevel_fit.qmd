...
function (problem::StateSpaceMod)(θ)
    ...
    fit!(mod1, save_hyperparameter_distribution = false)

    if !isnan(mod1.results.llk)
        loglikelihood = mod1.results.llk
    end
...
my_transform = as((r = asℝ₊, K = asℝ₊, x₀ = asℝ₊))
problem = StateSpaceMod(true_solution.ts, true_solution.y, my_priors, 3, my_transform)
...
```{julia}
#| code-fold: true
pairplot(chn, PairPlots.Truth(Base.structdiff(true_solution.parameter, (; σ_o = 0))))
```
...
let
    n = 100000
    prior_df = (;
        r = rand(my_priors.r, n),
        K = rand(my_priors.K, n),
        x₀ = rand(my_priors.x₀, n))
