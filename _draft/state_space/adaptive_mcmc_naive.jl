...
function ModelScratchMCMC(y, ts)
    prior_dists = (;
        β = Uniform(0, 1),
        σ = truncated(Normal(0.0, 1.0); lower = 0),
        x₀ = truncated(Normal(0.0, 10.0); lower = 0))
    transform = as((β = as𝕀, σ = asℝ₊, x₀ = asℝ₊))
...
function (problem::ModelScratchMCMC)(θ)
    ...
    lprior += logpdf(prior_distributions[:σ], σ)
    ...
    if lprior > -Inf
        ...
        for t in ts
            x = β * x
            llikelihood += logpdf(Normal(x, σ), y[t])
        end
    end
...
for i in 1:nchains
    raw_post_chain = adaptive_rwm(sample_prior(problem), lposterior, nsamples;
                            b = nsamples ÷ 2, thin = 1, progress = true);
...
PairPlots.pairplot(chn_mcmc,
    PairPlots.Truth( (;σ = 3.0, β = 0.95, x₀ = 20.0)))
